...
export async function singleTokenDeposit(
  ...
  let usdcMarketInfo = MARKETS[MARKETS.map(m => { return m.name }).lastIndexOf(pairSymbol)];
  if (usdcMarketInfo.deprecated) { throw "Chosen Market is deprecated" };
  let usdcMarketData = await getMarketData(connection, usdcMarketInfo.address);

  let [marketUSDC, midPriceUSDC] = await getMidPrice(connection, usdcMarketInfo.address);
  ...
  for (let asset of poolAssets) {
    ...
    if (!sourceAssetInfo) {
      createAssetInstructions.push(await createAssociatedTokenAccount(
  ...
  let openOrder = openOrders[openOrders
    .map(o => o.market.toString()).indexOf(marketUSDC.address.toString())
  ];
  ...
  for (let asset of poolAssets) {
    ...
    let poolAssetBalance = +(await connection.getTokenAccountBalance(poolAssetKey)).value.amount;
  ...
  for (let i=0; i < poolAssets.length; i++) {
    let poolAssetSymbol = TOKEN_MINTS[
      TOKEN_MINTS.map(t => t.address.toString()).indexOf(poolAssets[i].mintAddress.toString())
    ].name;
    let assetPairSymbol = poolAssetSymbol.concat("/USDC");
    
    let assetMarketInfo = MARKETS[MARKETS.map(m => { return m.name }).lastIndexOf(assetPairSymbol)];
    if (assetMarketInfo.deprecated) { throw "Chosen Market is deprecated" };
  
    let [assetMarket, assetMidPrice] = await getMidPrice(connection, assetMarketInfo.address);
    poolAssetMarkets.push(assetMarket);
    let assetAmountToBuy = midPriceUSDC * amount * poolAssetAmounts[i] / (assetMidPrice * totalPoolAssetAmount);
    poolTokenAmount = Math.max(poolTokenAmount, assetAmountToBuy / poolAssetAmounts[i]);
  ...
  for (let i=0; i < poolAssets.length; i++) {
    ...
    let openOrder = openOrders[openOrders
      .map(o => o.market.toString()).indexOf(assetMarket.address.toString())
    ];
    let sourceUSDCKey = await findAssociatedTokenAddress(
      sourceOwner.publicKey,
      marketUSDC.quoteMintAddress
  ...
  if (Object.is(targetInfo, null)) {
    instructions.push(await createAssociatedTokenAccount(
      SystemProgram.programId,
      payer.publicKey,
      sourceOwner.publicKey,
      poolMintKey
